...
class S3Base(object):
	...
	def store_raw_data_in_s3(self, partition_date, data_type, data, personal, mandator, export_bucket, parquet=False, chunk=0, chunk_name=False):
		...
		if parquet:
			...
			data_key = self.create_data_key(partition_date, str(data_type), data_type, mandator, personal=personal, parquet=parquet)
			if chunk_name:
				file_name = f"{str(chunk)}_{file_name}"
			s3_uri = self.create_s3_uri(export_bucket, data_key, file_name, FileType= "parquet")
			self.upload_parquet_to_s3(s3_uri, data)
		else:
			data_key = self.create_data_key(partition_date, f"{str(data_type)}.jsonp", data_type, mandator, personal=personal)
	...
	def store_report_log_in_s3(self, export_type, data, process_start_date, index, data_type, mandator, log_bucket= None):
		...
		'''
		if log_bucket:
			report_log_key = self.create_report_log_key(process_start_date, export_type, index, str(data_type), mandator)
	...
	def delete_raw_master_data_in_s3(self, data_type, personal, export_bucket, mandantor, parquet=None):
		...
		'''
		if parquet:
			partial_data_key_deletion = self.create_partial_data_key_deletion(data_type, mandantor, personal=personal, parquet=parquet)
